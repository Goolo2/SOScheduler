...
from simulators.ForestElements import Tree
...
class LatticeForest(Simulator):
    ...
    def __init__(self, dimension, rng=None, initial_fire=None,
        ...
        if tree_model == 'exponential':
            self.alpha = defaultdict(lambda: 0.2763) if alpha is None else defaultdict(lambda: alpha)
        elif tree_model == 'linear':
            self.alpha = defaultdict(lambda: 0.2) if alpha is None else defaultdict(lambda: alpha)
        self.beta = defaultdict(lambda: np.exp(-1/10)) if beta is None else defaultdict(lambda: beta)
        ...
        self.fire_center = (r_center, c_center)
